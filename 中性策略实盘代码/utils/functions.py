...
def cal_factors(df, all_factor_list, run_time, hold_period):
    # 遍历每个因子，计算每个因子的数据
    ...
    # 只保留需要的字段
    df = df[['candle_begin_time', 'symbol', 'close'] + feature_list]
...
def cal_factor_and_select_coin(symbol_candle_data, strategy, run_time, kline_size=99):
    ...
    for df in symbol_candle_data.values():
        df = cal_factors(df, strategy['factors'], run_time, hold_period)
        if df is not None and not df.empty:
            all_df_list.append(df)

    # ===合并并整理所有K线
    all_df = pd.concat(all_df_list, ignore_index=True)
    all_df.dropna(subset=feature_list, inplace=True)  # 删除选币因子为空的数据
    ...
    all_df = all_df[['candle_begin_time', 'symbol', 'close', '因子']]  # 只保留需要的字段

    # ===选币
    all_df = select_long_and_short_coin(all_df, strategy['select_coin_num'])

    # ===筛选合适的offset
    all_df['offset'] = all_df['candle_begin_time'].dt.hour % hold_period  # 计算每个周期属于的offset
    all_df = all_df[all_df['offset'].isin(strategy['offset_list'])]  # 保留配置中的offset数据
...
def cal_order_amount(position_df, select_coin, strategy, equity, leverage):
    ...
    :param equity:                  账号交易金额
    :param leverage:                杠杆
    :param symbol_last_price:       最新币价数据
    :return:
