...
def load_market(exchange, black_list):
    ...
    symbol_list = [symbol for symbol in symbol_list if symbol.endswith('USDT')]  # 挑选出所有usdt合约的交易对
    # 删除在黑名单中的币种
    ...
    for info in exchange_info['symbols']:
        ...
        for filter in info['filters']:
            if filter['filterType'] == 'PRICE_FILTER':
                price_precision[symbol] = int(math.log(float(filter['tickSize']), 0.1))
            elif filter['filterType'] == 'LOT_SIZE':
                min_qty[symbol] = int(math.log(float(filter['minQty']), 0.1))
            elif filter['filterType'] == 'MIN_NOTIONAL':
                min_notional[symbol] = float(filter['notional'])
